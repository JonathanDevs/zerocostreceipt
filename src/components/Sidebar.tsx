...
export default function Sidebar({ activeTab, setActiveTab, receiptCount, mobileSidebarOpen }: SidebarProps) {
  ...
  return (
    <aside
      ...
      className={`fixed top-0 left-0 z-40 w-64 h-screen transition-transform bg-white text-neutral-800 flex flex-col justify-between border-r border-[#E5E7EB] ${
    ...
    >
      <div className="px-5 py-6">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-base font-bold tracking-tight text-[#111827] leading-tight">ZeroCost</h1>
            <span className="text-[11px] font-semibold text-[#6B7280]">Procesador IA $0 Cost</span>
          </div>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <IconComponent className={`w-4 h-4 transition-transform duration-150 ${isActive ? 'text-black' : 'text-[#6B7280] group-hover:text-black'}`} />
                  <span>{item.label}</span>
                </div>
                {item.badge !== undefined && (
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${
                    isActive ? 'bg-black text-white' : 'bg-[#E5E7EB] text-[#4B5563]'
                  }`}>
                    {item.badge}
                  </span>
                )}
      ...
      <div className="p-5 mt-auto">
        <div className="bg-[#F3F4F6] p-4 rounded-xl space-y-2.5">
          <p className="text-[10px] text-[#6B7280] uppercase font-bold tracking-wider leading-none">Estado API</p>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div>
            <span className="text-xs font-semibold text-[#111827]">Gemini Free Tier</span>
          </div>
          <div className="text-[10px] text-[#9CA3AF] font-mono leading-none border-t border-[#E5E7EB] pt-2.5 flex justify-between">
